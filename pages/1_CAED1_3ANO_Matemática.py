...
st.title("👥 CESB-Recomposição de Aprendizagem")
...
def carregar_dados():
    ...
    try:
        ...
        csv_path = os.path.join(script_dir, 'CAED1_3_matematica.csv')
...
if df is not None:
    try:
        ...
        habilidades_cols = [col for col in df.columns if col.startswith('H ') and int(col.split()[1]) <= 18]
        ...
        if tipo_analise == "Buscar Aluno Específico":
            ...
            fig.update_layout(
                polar=dict(
                    radialaxis=dict(
                        ...
                        range=[0, 2],
                        tickvals=[0, 1, 2],
                        ticktext=['Não Domina', 'Domina', 'Domina<br>Plenamente'],
            ...
            with col_nivel:
                nivel_selecionado = st.selectbox(
                    "Selecione o nível:",
                    ["❌ Não Domina (0)", "⚠️ Domina (1)", "✅ Domina Plenamente (2)"]
                )
            ...
            nivel_desc = get_nivel_descricao(nivel_valor).replace('❌ ', '').replace('⚠️ ', '').replace('✅ ', '')
            ...
            with col_filtro2:
                grupos_filtro = st.multiselect(
                    "Filtrar por grupos:",
                    ["❌ Não Domina", "⚠️ Domina", "✅ Domina Plenamente"],
                    default=["❌ Não Domina", "⚠️ Domina", "✅ Domina Plenamente"]
                )
            ...
            heatmap_data = stats_df.set_index('Habilidade')[['% Não Domina', '% Domina', '% Domina Plenamente']]
            fig = px.imshow(
                heatmap_data.T,
